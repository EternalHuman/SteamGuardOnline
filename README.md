...
Код доступа можно передать при открытии сайта через `?id=<code>`. После чтения параметр очищается из адресной строки; в cookie, localStorage и тело API-запроса открытый код не записывается. На сервер отправляется только производный 256-битный token.
